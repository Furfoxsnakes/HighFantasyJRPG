...
public class NotificationCenter
{
    ...
    /// </summary>
    private readonly Dictionary<string, SenderTable> _table = new Dictionary<string, SenderTable>();
    ...
    public void AddObserver(Handler handler, string notificationName, object sender)
    {
        if (handler == null)
        {
            GD.PrintErr("Can't add a null event handler for notification, " + notificationName);
            return;
        }

        if (string.IsNullOrEmpty(notificationName))
        {
            GD.PrintErr("Can't observe an unnamed notification");
            return;
        }
    ...
    public void RemoveObserver(Handler handler, string notificationName, object sender)
    {
        if (handler == null)
        {
            GD.PrintErr("Can't remove a null event handler for notification, " + notificationName);
            return;
        }

        if (string.IsNullOrEmpty(notificationName))
        {
            GD.PrintErr("A notification name is required to stop observation");
            return;
        }
    ...
    public void PostNotification(string notificationName, object sender, object e)
    {
        if (string.IsNullOrEmpty(notificationName))
        {
            GD.PrintErr("A notification name is required");
            return;
        }
